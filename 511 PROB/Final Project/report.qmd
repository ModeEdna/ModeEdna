---
title: "Comparison of Music Taste Across Regions and Generations"
...
-   For all our code and data, take a look at out [GitHub repository](https://github.com/ModeEdna/511FinalProject).

## Project intro

Given that music has, is, and will continue to be one of the strongest influences in human culture/life, it has given way to plenty of strong opinions. These include - which decade has the best music, what makes music good, etc. One of the reasons for such debate has been the evolution of sound, and how it has changed music. The problem with these comparisons is that they're all subjective. To remove subjectivity from the topic, we want to answer some questions using statistical methods.

After some brainstorming, we settled on 4 topics:

1.  Do the characteristics of popular songs vary between countries that speak the same language?

2.  Has music become louder over time?

3.  Does music in Mexico's Top 50 playlist tend to be more danceable and happy than the US counterpart?

4.  Are there any song characteristics that can help predict popularity scores?

The intention of this project is to compare the taste of two nations that speak the same language, find if songs have gotten louder over the past 7 decades, understand if variables vary depending on language and culture, and to find if feature scores can affect the popularity score or if popularity is more of an abstract concept that can't be predicted based on the available variables.

# Data

## Gathering

With the aforementioned topics in mind, we quickly understood that Spotify's API would help us get all the data we need. With the API, we retrieved information on the following playlists: Mexico Top 50, USA Top 50, UK Top 50, music from each decade in the time span 1950-2020, and last but not least, a personally-curated playlist with over 950\~ songs that we would use for a regression model. The data would be returned in a data frame format, with over 50 variables per song. We decided to remove most of them and keep the ones that would help us answer the questions we were interested in. The following data frame shows the result from the data pull of Mexico's Top 50 songs.
...
To answer this question, we will explore the top 50 most popular songs playlists from the United States and the United Kingdom. We wanted to see if there is a difference in the most popular songs, even though they speak the same language and would be expected to listen to similar music and genres. Typically, the United Kingdom has very upbeat songs, such as pop, dance, and EDM, topping the charts each week. On the other hand, the United States seems to favor other types of music, like rap, country, and soft-pop. For this analysis we will compare danceability, energy, and valence of the top 50 songs. We will use a two-sample t-test for each of the three variables.
...
First, we look into the summary statistic of each variable of interest for each playlist.
...
Comparison of Danceability - very minimal difference in the mean of danceability. UK is 0.64 and USA is 0.63. We expected the UK mean to be higher, as their charts often reflect more dance/edm/pop songs ("club music").

Comparison of Energy - somewhat significant difference in the mean of energy. UK is higher at 0.62 and USA is lower at 0.54.

Comparison of Valence - somewhat significant difference in the mean of valence. UK is higher at 0.5 and USA is lower at 0.4.
...
The danceability boxplot (far-left) shows that there isn't much difference at all between the countries. The median danceability is slightly higher for the USA (0.65) than the UK (0.64). The interquartile range is also slightly larger for the USA than the UK.
...
In Figure 1.2, we can visualize the distribution of each variable. Each histogram is shown by country, with blue representing the UK and red representing the USA.
...
Null hypothesis - There is no difference in the average danceability between the UK and the USA.

Alternative hypothesis - The average danceability is larger for the UK than the USA.
...
Interpretation - Since p-value is greater than 0.05, at 5% significance level, we have enough evidence to conclude that we fail to reject the null hypothesis. Therefore we can say that the danceability of music in the top 50 in the UK is higher than music in the top 50 in the USA.

------------------------------------------------------------------------

Null hypothesis - There is no difference in the average energy between the UK and the USA.

Alternative hypothesis - The average energy is larger for the UK than the USA.
...
Interpretation - Since p-value is less than 0.05, at 5% significance level, we have enough evidence to reject the null hypothesis. Therefore we believe that the energy of music in the top 50 in the UK is higher than music in the top 50 in the USA.
...
The 95% bootstrap percentile confidence interval for the ratio of means (UK energy / USA energy) is from 1.03 to 1.32. Thus, we are 95% confident that the energy of the top 50 UK songs are, on average, between 1.03 and 1.32 times greater than the top 50 USA songs.
...
Null hypothesis - There is no difference in the average valence between the UK and the USA.

Alternative hypothesis - The average valence is larger for the UK than the USA.
...
Interpretation - Since p-value is less than 0.05, at 5% significance level, we have enough evidence to reject the null hypothesis. Therefore we conclude that the valence of music in the top 50 in the UK is higher than music in the top 50 in the USA.
...
The 95% bootstrap percentile confidence interval for the ratio of means (UK valence / USA valence) is from 1 to 1.59. Thus, we are 95% confident that the valence of the top 50 UK songs are, on average, between 1 and 1.59 times greater than the top 50 USA songs.
...
After conducting each test, we noted some important findings regarding the difference in top 50 songs for the UK and the USA. We expected all three variables to be of higher value for the UK songs than the US songs, but that was only true for energy and valence. We thought that if two countries speak the same language, their music would be similar. However, we now believe that music preference stems from a country's culture.
...
Music can be considered a defining characteristic of a generation. The most popular songs of a decade often reflect the general culture, philosophy, and sentiment of that time. For this reason, our second question set out to examine how popular music has changed on a decade-by-decade basis. Specifically, we set out to see what variables have evolved most drastically, and whether identifying these changes might be informative with respect to trends in popular culture.

The first step in the process was to gather appropriate data to explore. Spotify maintains a playlist for each decade from 1950 to 2010 entitled "All out" followed by the year. These playlists reflect the most popular songs from that time in the United States. While Spotify is not transparent about their sampling method for the playlist, it is assumed that these playlists are based on the top charts from each decade. The 2020's were included for continuity, and in this case the Spotify "top tracks" playlists were used for each year of the decade thus far (2020, 2021, 2022). All data were gathered from the Spotify API, using the SpotifyR package. The final data set contained 9 features, one categorical decade label, and 1,329 songs (10 x 1329 matrix).

### EDA

To decide which variable we wanted to analyze, we plotted the means of the variables across the decades to find trends in movement. We then graphed the distributions of the loudness variable (feature we selected) for each time period.
...
Based on the EDA, one of the clearest trends identified in the first plot is within the loudness variable; music gets louder for 5 decades before peaking and decreasing. Loudness is also one of the most objective measures in the dataset since it is measured in decibels (dB). For these reasons, we selected loudness as the feature of interest. Next, the distribution of each decade's loudness score (in decibels) was also plotted to determine its suitability for parametric hypothesis tests. Each decade's loudness distribution appeared approximately gaussian, and we proceeded with the tests since they are robust to moderate deviations from normality.

### Testing

**ANOVA**

After EDA, we conducted an analysis of variance (ANOVA) test on the loudness scores of each decade. An ANOVA is an univariate test to determine whether a variable is different under the conditions of a categorical variable. This test will help us determine whether, in general, there is a difference in mean loudness among the decade groups. However, it will not inform us of the difference's directionality or which decades differ specifically. The null and alternative hypothesis are as follows:

$H_0: \mu_{1950} = \mu_{1960} = \mu_{1970} = \mu_{1980} = \mu_{1990} = \mu_{2000} = \mu_{2010} = \mu_{2020}$

Or in other words, there is no difference in mean loudness scores among the decades.
...
The difference in mean loudness scores for some decade X and decade Y are not zero. In other words, on average decade X and decade Y have different loudness levels on their most popular tracks.
...
The results of our pairwise t-tests suggest that for many decades, there is a significant difference in loudness of popular songs. Specifically, from the years 1970-2000, there is an increase in loudness score for the most popular tracks. However, there are no important differences in loudness for consecutive decades on the lower and upper range. For example, there is no significant change in loudness from 1950 to 1960, 1960 to 1970, and 2010 to 2020. Notably, between 2000 and 2020, there is a decrease in loudness for popular songs. Overall, this suggests a trend in popular music where songs have on average increased in volume from the period 1950-2000, and begun to decrease in the period 2000-2020.

In conclusion, the loudness in decibels of popular music has significantly shifted. We saw an increase in loudness for over 6 decades, and a decrease over the past 20 years. While the reasons for this trend may be difficult to determine, there are a few plausible hypotheses. Genre may have a large impact, but it is also worth considering that the popularity of genres is not necessarily directly reflected in top charts. Rock, disco, and rap were popular in the last decades of the 1900s, then genres like punk rock, R&B, and alternative became popular in the 00's. It is surprising that in the 2010s and 2020s, a time where loud electronic music has become more popular, we are starting to see a decrease in loudness. However, considering that top charts contain more pop music, and currently the most popular songs are from artists such as Taylor Swift, The Weeknd, Post Malone, and Lil Uzi Vert, it makes some intuitive sense that prominent music has become quieter.
...
To get an initial understanding of the data, we will have to do some exploration. We believe that the mean of our selected variables will be different based on the country's playlist they belong to, but some visualizations would help in seeing the relationships more clearly. Mexico is often seen as a happy country, while the US struggles a bit more in this department; we would expect these feeling to be reflected on the music they listen to. For this, we will be comparing the Valence score of the top 50 songs from each country. The other variable we'd like to compare would be the Danceability score. More often than not, people dance when they're happy. We would expected for Mexico's Danceability score to be higher than the US score.
...
Before trying a hypothesis test, we can make some quick observations using the visualizations. Within both plots, the black diamond represents the mean for the variable within their respective playlist. In both variables, the mean for Mexico is significantly higher than the mean for the US. Although Mexico's Danceability scores are noticeably more concentrated at the top of the graph, the scores for Valence have a more even distribution. Albeit, both variable means for Mexico are greatly higher than the ones for the US. If we had to make a conclusion using only EDA, we would conclude that the label does in fact affect the variable scores. But, we know better than to make conclusions based on EDA. That's why we'll do an MANOVA test.
...
By using a regression model, we can determine which (if any) variables contribute to predicting a song's popularity. This could be informative from a psychological perspective in that we can determine what features of a song contribute to it's broad appeal. It could also inform musicians who hope to make popular music on what styles they should include to provide them with their best chance at reaching a wide audience. The first step in our analysis is to fit a full regression model with all variables. We will only include the continuous song variables. As the distribution of decades is highly skewed, it was removed from analysis.

### EDA

We begin by plotting the distribution for popularity score. Next, we plot the histograms for certain variables we believe to be important. And lastly, we take a look at the distribution of songs by decade.
...
In Figure 4.1, we can see the distribution for song popularity in the playlist. On the left, the histogram shows that the mean popularity for the songs is 53; this indicates that the mean is moderate. We can also see the distribution is slightly left-skewed, signaling that the songs in the playlist might have an overall higher popularity.

On the right, the boxplot shows the median popularity for the songs is 57. We can also see that the interquartile range is between 39 and 70, showing that the popularity scores are compacted near the mean.
...
In Figure 4.2, we can see the distributions of six of our predictor variables. We chose to display these, as we believe they may have the most significant effect on our response variable. It is important to note that we will begin our testing with more predictor variables than shown.
...
In Figure 4.3, we can see a count of songs within the playlist by decade of release. Recall that the playlist may not be fully representative of every song on Spotify. Our playlist has a majority of songs from this millennium. In the barplot, it is clear that the 2000s and 2010s decades were the most represented in the playlist, while the 1950s and 1960s were the least represented. As new music is released, a user's overall songs and playlist may reflect this change, with newer music making up a higher proportion.
...
Before examining the full model, the data must be examined for the issue of multicollinearity. This is because predictor variables with strong correlations can each provide similar information to the model; adding strongly correlated features to a model will only increase the noise.
...
The first model was fitted using all variables and then we checked for collinearity between them.

```{r}
# fit1 is an lm model using all variables
car::vif(fit1)
```

In our initial regression model, the energy variable had a variance inflation factor (VIF) of 6.36, which is considered above the cutoff for multicollinearity. As a result, we will drop the energy variable from our regression.

------------------------------------------------------------------------

Our second model was fit after removing energy.
...
In our adjusted model, we found that the variables loudness, acousticness, and tempo were the significant predictors of popularity.

------------------------------------------------------------------------

For fit3 we ran an interaction model, which includes the relationship between features and their combined effect on the response variable.
...
In our interaction model, the only significant variable at the 95% confidence level was track duration. There were a few significant interaction terms, such as the interaction track duration:danceability, loudness:acousticness, acousticness:liveness, and instrumentalness:valence.
...
Following model creation, the next step was to assess our model performance using the metrics $R^2$, $RSE$, and $F$.
...
Given the performance measures, it is difficult to determine which model is the best. The F statistic, which tells us how significant our model is, is strongest for the second model, which is the one without interactions. However, the second model also has the lowest $R^2$ value, which shows how much of the response variable's variance can be explained by the model. The $RSE$ or Residual Standard Error is lowest on model 3, which also has the lowest F statistic. The $RSE$ is a measure of the residuals, or the distance from the predicted value; a lower $RSE$ indicates a better fit. To complete the rest of the analysis, and to determine whether a linear model is suitable for this data, we will use model 4, which has a balance of all performance measures. Model 4 was our trimmed model that included interactions.
...
In question 4, we thought it might be possible to predict a song's popularity based on specific variable scores. Unfortunately, we came to the realization that no such model could be constructed with linear regression. Factors affecting song popularity might be more abstract and difficult to pin down than by scoring songs based on their tempo, key, etc.

## Possible improvements

\- The most common improvement to any data science project is more data. Since we were using Top 50 playlists or playlists with less than 1000 songs, we might have not captured the full distributions of scores among songs.
...
\- Maybe other streaming services have more variables that we could have used to explore the differences between our entities.
...
# Authentication
my_token <- get_spotify_access_token(client_id = "cliend_id", client_secret = "client_secret")
...
# Authenticate
my_token <- get_spotify_access_token(client_id = "client_id", client_secret = "client_secret")
...
**Histograms of loudness across decades for question 2**
