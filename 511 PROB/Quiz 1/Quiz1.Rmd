...
**6a**. What would be a rough minimum "large enough" number of iterations that you need to get a good probability?

Answer: The chart shows that around 2000-2500 is a good amount of iterations.
...
**7a**. Use function from part 5 and function from part 2.
```{r 7a}
# function from part 2
labs <- c('I','ON', 'OS')
sims <- 100000
probslabel <- c(613/1527,579/1527,335/1527)
condps <- c(c(0.07,0.93), c(0.53,0.47), c(0.24,0.66))
...
**7c**. Compare the probabilities.

Comparison: looks to me like the functions from part 4 and 5 are slightly more similar than the last one. That being said, all three probabilities are within a 2% range, which in my opinion is a really good estimation.

**8a**. What can you say about using a Monte Carlo simulation for this problem?

I think it did a great job at predicting the probability that it was asked. I understand why this type of method is useful in the real world. Without all the information I used for other estimates, the Monte Carlo estimation was able to get a near-identical probability.

**8b**. Based on the data from these two sets, what is your conclusion? Bonus points for using more than 2 probabilities to answer the question.

We are certain that iPhone users will like the new feature, as NY and California both reported probabilities of over 82% in their respective area. However, the probability of a non-iPhone user to like the new feature (NY) is only 50.01%; this includes participants who are willing to change phones and who aren't willing to do so. Ideally, we want to target the non-iPhone users who are interested in switching to an iPhone, hoping to convert them. Their percentages of liking the new feature stand at 66% (CAL) and 49% (NY), so it would be best to find a way to get those percentages higher to get a high conversion rate.

A possible path to take would be focus groups. This way we can ask the would-be converters what they like about the feature and what would make them like it more.